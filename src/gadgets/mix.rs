...
pub fn fill_cs<CS: ConstraintSystem>(
    ...
    for i in 0..inputs.len() - 2 {
        ...
        inner_mix::fill_cs(cs, A, B, C, D.clone())?;
...
mod tests {
    ...
    fn mix_gadget() {
        ...
        assert!(mix_helper(vec![(6, peso, 0)], vec![(6, peso, 0)]).is_ok());
        // error when merging different asset types
        assert!(mix_helper(vec![(3, peso, 0)], vec![(3, yuan, 0)]).is_err());
