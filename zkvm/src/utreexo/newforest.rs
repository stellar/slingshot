//! New forest with dynamic CoW nodes
...
impl WorkForest {
    ...
    fn delete_committed<M: MerkleItem>(
        ...
        let top = find_root::<_, Rc<Node>>(self.roots.iter_mut(), path.position)
            .ok_or(UtreexoError::InvalidProof)?;
        ...
        let existing = path
            ...
            .try_fold(top, |node, (side, neighbor_hash)| {
                if let Some((ref mut l, ref mut r)) = node.children {
                    // mark the node as modified only if we actually descend,
                    // so the final available node is not marked too early.
                    // It will be marked as modified when we replace it
                    // with a root of a new subtree below.
                    node.modified = true;
...
fn find_root<I, B>(roots: I, position: Position) -> Option<I::Item>
where
    I: IntoIterator,
    I::Item: Borrow<B>, // this enables passing &Rc<Node> here
    B: Borrow<Node>,
{
    let mut offset: Position = 0;
    for item in roots.into_iter() {
        offset += 1u64 << item.borrow().borrow().level;
        if position < offset {
            return Some(item);
        }
    }
